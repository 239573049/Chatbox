export async function fetch(url: string, options: any) {
  ...
  try {
    const baseUrl = 'https://token-ai.cn';
    const fullUrl = url.startsWith('http') ? url : `${baseUrl}${url}`;
    const response = await window.fetch(fullUrl, { ...options, headers });
    ...
    if (response.status === 401) {
      const currentUrl = window.location.origin+'/oauth';
      const encodedReturnUrl = encodeURIComponent(currentUrl);
      localStorage.removeItem('token')
      window.location.href = `https://token-ai.cn/login?redirect=${encodedReturnUrl}`;
    }
